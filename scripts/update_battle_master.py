...
# バージョンは救えないため、seasonId を出さず旧データ互換（キー無し＝常に有効）で
# 配信する。
#
# 戻さない。ここに書く値は current_battle_season_id()、つまり「生成した時点の
# 現行シーズン」であって「この調整値がどのシーズンのものか」ではない。調整表が
# 古いまま新シーズンに入っても、seasonId だけ新しくなって素通りするため、守りたい
# ケースを守れない。一方でアプリ側のシーズン認識がずれると調整値が丸ごと落ちる
# （2026-09-03 に発生）。
#
# 古い調整値を落とす役目は、build_twilight_trails_overrides の控え（baseline）が
# 担う。マスターが発表前の値から動いた時点で、予想の当たり外れに関係なく落ちる。
# もしガードを復活させるなら、schedule ではなく調整表と一緒に手で更新する定数
# （例: MOVE_OVERRIDES_SEASON_ID）にすること。
