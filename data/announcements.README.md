...
- `updatePromptJa` / `updatePromptEn`: 任意。`recommendedBuild`未満のユーザーへ表示する更新促し文。
- `updateActionTitleJa` / `updateActionTitleEn`: 任意。更新ボタンの文言。未指定時はアプリ側の既定文言。
- `updateAppStoreUrl`: 任意。更新ボタンの遷移先。iOSアプリはGBLBOXのApp Store URLだけを受け付ける。
